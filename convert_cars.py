...
def arrange_cars_data(anno_filename, dataset_dir):
    """dadaset_dir: dir where `car_ims` is in."""
    # Load label information
    matrix = loadmat(anno_filename)

    # create as many directories as the number of classes in dataset (196)
    import os
    for idx in range(matrix['class_names'].shape[1]):
        os.makedirs(os.path.join(dataset_dir, 'raw_data', str(idx + 1)))
    ...
    for idx in range(matrix['annotations'].shape[1]):
        class_number.append(
            matrix['annotations'][0][idx][5][0][0])  # index 5 has class number
        filenames.append(
            matrix['annotations'][0][idx][0][0])  # index 0 has the class name

    # use the class name to fetch file from source directory and copy it in
    # corresponding class number directory
    import shutil
    for idx, _ in enumerate(class_number):
        shutil.copy2(os.path.join(dataset_dir, filenames[idx]),
                     os.path.join(dataset_dir, 'raw_data',
                                  str(class_number[idx])))

    # rename each directory from class number to actual class name
    import os
    for idx, _ in enumerate(class_number):
        if os.path.exists(
                os.path.join(dataset_dir, 'raw_data', str(class_number[idx]))):
            os.rename(
                os.path.join(dataset_dir, 'raw_data', str(class_number[idx])),
                os.path.join(dataset_dir, 'raw_data',
                             matrix['class_names'][0][
                                 class_number[idx] - 1][0].replace(' ',
                                                                   '_').replace(
                                 '/', '-')))
...
def _get_filenames_and_classes(dataset_dir):
    ...
    """
    flower_root = os.path.join(dataset_dir, 'raw_data')
    directories = []
    class_names = []
...
def run(dataset_dir):
    ...
    num_validation = int(VALIDATION_RATIO * len(photo_filenames))
